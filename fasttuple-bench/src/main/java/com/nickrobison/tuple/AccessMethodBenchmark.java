...
public class AccessMethodBenchmark {
    ...
    private PoolSettings<Container> poolSettings = new PoolSettings<>(new PoolableObjectBase<Container>() {
        @Override
    ...
    public AccessMethodBenchmark() {
        try {
            ...
            schema = TupleSchema.builder().addField("a", Long.TYPE).addField("b", Integer.TYPE)
                    .addField("c", Short.TYPE).implementInterface(StaticBinding.class).directMemory().build();
            ...
            pool3 = new TuplePool<>(10, false, size -> {
    ...
    public long testInvokeDynamic() throws Throwable {
        ...
        return (Long) mhga.dynamicInvoker().invoke(container) + (Integer) mhgb.dynamicInvoker().invoke(container)
                + (Short) mhgc.dynamicInvoker().invoke(container);
    }
